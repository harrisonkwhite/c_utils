...
s_texture_group GenTextures(const int tex_cnt, const t_gen_texture_info_func gen_tex_info_func, s_gl_resource_arena* const gl_res_arena, s_mem_arena* const mem_arena, s_mem_arena* const temp_mem_arena) {
    ...
    const s_gl_id_array gl_ids = ReserveGLIDs(gl_res_arena, tex_cnt, ek_gl_resource_type_texture);
    ...
    for (int i = 0; i < tex_cnt; i++) {
        const s_texture_info tex_info = gen_tex_info_func(i, temp_mem_arena);
...
s_rect_edges TextureCoords(const s_rect_int src_rect, const s_v2_int tex_size) {
    assert(IsSrcRectValid(src_rect, tex_size));
    assert(tex_size.x > 0 && tex_size.y > 0);
...
static void LoadFontArrangementInfo(s_font_arrangement_info* const arrangement_info, const stbtt_fontinfo* const stb_font_info, const int height) {
...
static t_gl_id GenShaderFromSrc(const char* const src, const bool frag, s_mem_arena* const temp_mem_arena) {
    assert(src);
    assert(temp_mem_arena && IsMemArenaValid(temp_mem_arena));
...
static t_gl_id GenShaderProg(const s_shader_prog_gen_info* const gen_info, s_mem_arena* const temp_mem_arena) {
    ...
    const t_gl_id vs_gl_id = GenShaderFromSrc(vs_src, false, temp_mem_arena);
    ...
    const t_gl_id fs_gl_id = GenShaderFromSrc(fs_src, true, temp_mem_arena);
...
static bool AttachFramebufferTexture(const t_gl_id fb_gl_id, const t_gl_id tex_gl_id, const s_v2_int tex_size) {
    assert(fb_gl_id != 0);
    assert(tex_gl_id != 0);
    assert(tex_size.x > 0 && tex_size.y > 0);
...
s_surface_group GenSurfaces(const int surf_cnt, const s_v2_int* const surf_sizes, s_gl_resource_arena* const gl_res_arena, s_mem_arena* const mem_arena) {
    ...
    const s_gl_id_array fb_tex_gl_ids = ReserveGLIDs(gl_res_arena, surf_cnt, ek_gl_resource_type_texture);
